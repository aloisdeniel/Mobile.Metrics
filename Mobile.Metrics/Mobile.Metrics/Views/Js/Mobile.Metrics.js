...
defaultActiveColors = ["#2780E3", "#3fB618", "#9954BB", "#FF7518", "#FF0039"];
...
function renderGraph(graph, colors) {
    ...
    if (graph.graph === "donut") {
        new Morris.Donut({ element: graph.id + "-graph", data: graph.data, colors: activeColors });
    }
    else if (graph.graph === "bars") {
        new Morris.Bar({ stacked: true, element: graph.id + "-graph", data: graph.data, xkey: graph.xkey, ykeys: graph.ykeys, labels: graph.labels, barColors: activeColors, xLabelAngle: 50 });
    }
...
function renderLegend(graph, colors) {
    ...
    Mustache.parse(template);
    var rendered = Mustache.render(template, viewModel);
    $('#'+graph.id).append("<div>"+rendered+"</div>");
}
